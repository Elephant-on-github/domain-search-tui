pub const TLDS: &[&str] = &[
    "com", "io", "dev", "app", "net", "org", "co", "me", "ai", "xyz", "info", "shop", "store",
    "cloud", "tech", "online", "site", "pro", "gg", "to",
];
...
pub const REGISTRAR_URLS: &[(&str, &str)] = &[
    ...
    (
        "cloudflare",
        "https://domains.cloudflare.com/?domain={domain}",
    ),
    ...
    (
        "mchost",
        "https://mchost.com/services/domains/check/?domain={domain}",
    ),
    ("sav", "https://www.sav.com/?domain={domain}"),
    (
        "ovhcloud",
        "https://www.ovhcloud.com/en/domains/domain-name-checker/",
    ),
    (
        "ionos",
        "https://www.ionos.com/domains/domain-check?domainName={domain}",
    ),
    ("one", "https://www.one.com/en/domain/?q={domain}"),
    (
        "infomaniak",
        "https://www.infomaniak.com/en/domains/find?q={domain}",
    ),
    (
        "gname",
        "https://www.gname.com/domain_check?domain={domain}",
    ),
    (
        "domgate",
        "https://www.domgate.com/domaincheck?domain={domain}",
    ),
    (
        "101domain",
        "https://www.101domain.com/domain_search.htm?domain={domain}",
    ),
    ("inwx", "https://www.inwx.com/en/domain/search"),
    (
        "ovh",
        "https://www.ovh.com/us/domains/domain_search/?search={domain}",
    ),
    (
        "atakdomain",
        "https://www.atakdomain.com/en/domain/search?q={domain}",
    ),
    (
        "onlydomains",
        "https://www.onlydomains.com/domain-search?q={domain}",
    ),
    ("gandi", "https://www.gandi.net/en/domain/search?q={domain}"),
    (
        "namesilo",
        "https://www.namesilo.com/domain/search-domains?q={domain}",
    ),
    (
        "openprovider",
        "https://www.openprovider.com/domain-check/?domain={domain}",
    ),
    ("loopia", "https://www.loopia.com/domain/?q={domain}"),
    (
        "hexonet",
        "https://www.hexonet.net/domain-check?domain={domain}",
    ),
    (
        "planethoster",
        "https://www.planethoster.com/en/Domain-Search?q={domain}",
    ),
    (
        "netim",
        "https://www.netim.com/en/domain-name-search?q={domain}",
    ),
    ("above", "https://www.above.com/domain-search/?q={domain}"),
    (
        "squarespace",
        "https://www.squarespace.com/domain-search?q={domain}",
    ),
    ("namecom", "https://www.name.com/domain/search/{domain}"),
    ("bigrock", "https://www.bigrock.in/domain-search?q={domain}"),
    (
        "blacknight",
        "https://www.blacknight.com/domain-names/?q={domain}",
    ),
    ("dreamhost", "https://www.dreamhost.com/domains/?q={domain}"),
    (
        "hostinger",
        "https://www.hostinger.com/domain-name-search?q={domain}",
    ),
    ("wix", "https://www.wix.com/domain/names?q={domain}"),
    ("wordpress", "https://wordpress.com/domains/?q={domain}"),
    (
        "dinahosting",
        "https://www.dinahosting.com/en/domains/search?q={domain}",
    ),
    (
        "network-solutions",
        "https://www.networksolutions.com/domain-name-search?q={domain}",
    ),
    (
        "internetbs",
        "https://internetbs.net/en/domain-search?q={domain}",
    ),
    ("uk2", "https://www.uk2.net/domain-check/?q={domain}"),
    (
        "123-reg",
        "https://www.123-reg.co.uk/domain-search?q={domain}",
    ),
    (
        "encirca",
        "https://www.encirca.com/domain-search/?q={domain}",
    ),
    (
        "instra",
        "https://www.instra.com/en/domain-names/domain-name-search?domain={domain}",
    ),
...
mod tests {
    ...
    fn test_registrar_search_url_prefills_domain() {
        ...
        assert_eq!(
            url, "https://porkbun.com/checkout/search?q=example.com",
        ...
        assert_eq!(
            url, "https://domains.cloudflare.com/?domain=example.com",
        ...
        assert_eq!(
            url, "https://www.name.com/domain/search/example.com",
        ...
        assert_eq!(
            url, "https://www.godaddy.com/domainsearch/find?domainToCheck=example.com",
    ...
    fn test_registrar_search_url_missing_slug() {
        assert!(
            registrar_search_url("nonexistent-slug", "x.com").is_none(),
    ...
    fn test_all_tracked_registrars_have_urls() {
        for reg in REGISTRARS {
            ...
            assert!(
                url.is_some(),
                "tracked registrar {} is missing a URL template",
                reg.slug
            );
        }
    }
    ...
    fn test_adjusted_price() {
        ...
        assert!(
            (preferred - 9.50).abs() < 0.001,
            "preferred 10.00 should become 9.50"
        );

        let unpreferred = adjusted_price(raw, "regery");
        assert!(
            (unpreferred - 10.00).abs() < 0.001,
            "non-preferred 10.00 should stay 10.00"
        );
    }
